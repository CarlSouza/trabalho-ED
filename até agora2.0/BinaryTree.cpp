...
// função auxiliar para calcular a altura de um nó especificado
int BinaryTree::height(TreeNode* node) {
    if (node == nullptr) {
        return 0;  // se o nó é nulo, a altura é 0
    } else {
        // calcular a altura de cada subárvore
        int leftHeight = height(node->ptrLeft);
        int rightHeight = height(node->ptrRight);

        // a altura do nó é o maior das alturas das subárvores + 1
        return std::max(leftHeight, rightHeight) + 1;
    }
}

// função auxiliar para calcular o tamanho de um nó especificado
int BinaryTree::size(TreeNode* node) {
    if (node == nullptr) {
        return 0;  // se o nó é nulo, o tamanho é 0
    } else {
        // o tamanho do nó é o tamanho das subárvores + 1
